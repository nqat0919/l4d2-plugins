...
char g_sCvar_BeamColor[12];
int g_iCvar_Alpha;
int g_iCvar_rgbaColorArray[4];
...
public void OnPluginStart()
{
	...
	g_hCvar_BeamColor = CreateConVar("l4d2_item_marker_beam_color", "255 255 0", "Color of the beam ring and glow\nUse three values between 0-255 separated by spaces (\"<0-255> <0-255> <0-255>\").", FCVAR_NOTIFY);
	g_hCvar_SpriteColor = CreateConVar("l4d2_item_marker_sprite_color", "255 0 0", "Color of the sprite\nUse three values between 0-255 separated by spaces (\"<0-255> <0-255> <0-255>\").", FCVAR_NOTIFY);
	...
	//Hook Cvar change
	g_hCvar_BeamColor.AddChangeHook(Event_ConVarChanged);
...
void GetCvars()
{
	...
	g_hCvar_BeamColor.GetString(g_sCvar_BeamColor, sizeof(g_sCvar_BeamColor));
	TrimString(g_sCvar_BeamColor);
	g_iCvar_Alpha = g_hCvar_Alpha.IntValue;
	GetArrayColor(g_sCvar_BeamColor, g_iCvar_rgbaColorArray);
	g_iCvar_rgbaColorArray[3] = g_iCvar_Alpha;
...
void DoMarkItem(int client, int iEntity, const char[] sEntModelName, char sItemName[64], const float vOrigin[3])
{
	...
	int iGlow = _create_EntityGlow_(iEntity, g_sCvar_BeamColor, sEntModelName);
	int iTarget = _create_InfoTarget_(iEntity, vOrigin, sTargetName, g_fCvar_MarkDuration);
	Handle hBeamTimer = _create_BeamTimer_(client, g_iCvar_rgbaColorArray, g_iCvar_ModelBeamIndex, iEntity, vOrigin);
	int iSprite = _create_Sprite_(client, iTarget, vOrigin, g_sCvar_SpriteColor, sTargetName, g_fCvar_MarkDuration, g_sCvar_ModelSprite);
...
void GetArrayColor(char[] sColor, int buffer[4])
{
    if (sColor[0] == 0)
        return;
